...
export default {
  ...
  methods: {
    async fetchCollectedPosts() {
      ...
      try {
        const response = await apiRequest(`collects/queryAllByUserId?userId=${userId}`, 'get');
        // const postIds = response.map(post => post.postId).join(',');
        const postsResponse = await apiRequest(`posts/postIds?postIds=${response}`, 'get');
        
        this.collectedPosts = await Promise.all(postsResponse.map(async post => {
